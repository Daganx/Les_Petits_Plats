...
function createIngredientListItem(ingredient) {
    ...
    li.addEventListener('click', () => handleIngredientClick(ingredient, li)); // Ajouter un gestionnaire de clic
    return li;
}

// Fonction pour gérer le clic sur un ingrédient
function handleIngredientClick(ingredient, li) {
...
function addIngredient(ingredient, activeTagsList, li) {
    ...
    const span = createActiveTag(ingredient); // Créer une balise active pour l'ingrédient
    activeTagsList.appendChild(span);
    li.classList.add('selected');
}

// Fonction pour créer une balise active pour un ingrédient
function createActiveTag(ingredient) {
...
function addSearchFunctionality() {
    document.getElementById('dropdown-search-ingredients').addEventListener('input', function(event) {
